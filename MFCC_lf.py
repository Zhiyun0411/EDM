...
class RMSE_MFCC_lf:
	...
	def train(self):
		...
		for xx in range(paraDict['maxIter']):
			...
			for std in list(trainingSet.keys()):
				...
				for tempTerm in list(tempStdVec.keys()):
					...
					for crsCode, testGrd in tempTermVec:
						...
						sum1=testGrd-np.dot(P[std],Q[crs])-paraDict['isbs']*bs[std]-paraDict['isbc']*bc[crs]
						...
						coCrs = np.zeros(K)
						coCrsSum = np.zeros(K)
						for coCrsCode, coCrsGrd in tempTermVec:
						...
						sum1=testGrd-np.dot(P[std],Q[crs])-paraDict['isbs']*bs[std]-paraDict['isbc']*bc[crs]-self.lf*leaveDMout
						...
						#
						predGrd = np.dot(P[std],Q[crs]) + paraDict['isbs']*bs[std] + paraDict['isbc']*bc[crs] + leaveDMout
			...
			testrmse, testmae, testpct0, testpct1, testpct2, testpredGrdGrp_crs, testpredGrdGrp_mjr = self.testMFCC(P,Q,bs,bc)
			#
			trainrmse, trainmae, trainpct0, trainpct1, trainpct2 = self.testRMSE(predGrdVec, trueGrdVec)
			print(' MFCC - ',xx,' time -- %.6f   '%(time.time()-oldtime),'train --  mae -- %.6f'%(trainmae), ' ///   test --  mae -- %.6f'%(testmae), '  pct0 -- %.6f'%(testpct0),'  pct1 -- %.6f'%(testpct1),'  pct2 -- %.6f'%(testpct2))
		...
		for std in list(testSet.keys()):
			...
			for tempTerm in list(tempStdVec.keys()):
				...
				for crsCode, testGrd in tempTermVec:
					...
					predGrd = np.dot(P[std],Q[crs]) + paraDict['isbs']*bs[std] + paraDict['isbc']*bc[crs] + self.lf*predGrd
